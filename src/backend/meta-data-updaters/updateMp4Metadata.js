...
Description:
  This script processes MP4 files in the specified input folder, updates their metadata using ExifTool, and saves the modified files to the output folder. Metadata includes title, description, keywords, copyright, genre, artist, software, and timestamps. Logs are generated for debugging and stored in the ./logs directory.

Arguments:
  <inputFolder>  (Required) The path to the folder containing input MP4 files.
  <outputFolder> (Required) The path to the folder where processed MP4 files will be saved.
...
Outputs:
  ...
  - Processed MP4 files: Saved to the specified <outputFolder> with updated metadata.

Example:
  node ${scriptName} ./input-mp4s ./output-mp4s
  node ${scriptName} ./input-mp4s ./output-mp4s -v
...
if (args.length < 2 || args.length > 3) {
    console.error('Usage: node updateMp4Metadata.js <inputFolder> <outputFolder> [-v]');
    process.exit(1);
}
...
const TITLE = "Modern White Kitchen Tour";
const DESCRIPTION = "Tour a modern white kitchen with marble countertops and sleek cabinets. Perfect for home decor inspiration.";
const KEYWORDS = "kitchen, interior design, home decor, marble countertops, modern kitchen";
const COPYRIGHT = "© 2025 Mynza"; // Already correct, no change needed
const GENRE = "Home Decor";
const ARTIST = "Mynza Team";
const SOFTWARE = "Adobe Premiere Pro 2023";
...
function updateMp4Metadata(inputPath, outputPath) {
    ...
    const command = [
        ...
        '-overwrite_original', // Overwrites temp file during processing
        `"${inputPath}"`
    ].join(' ');
...
// Function to process all MP4 files in the input folder
function processMp4Folder(inputFolder, outputFolder) {
    ...
    if (files.length === 0) {
        logToFile(`No MP4 files found in ${inputFolder}`);
        return;
    }
    ...
    files.forEach(file => {
        ...
        updateMp4Metadata(inputFilePath, outputFilePath);
    });

    logToFile('All MP4 files processed.');
    logToFile('Script completed');
}

// Run the script
processMp4Folder(INPUT_FOLDER, OUTPUT_FOLDER);