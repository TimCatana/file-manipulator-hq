...
Description:
  This script processes JPEG files in the specified input folder, updates their metadata using ExifTool, and saves the modified files to the output folder. Metadata includes title, description, keywords, copyright, genre, and timestamps. Logs are generated for debugging and stored in the ./logs directory.

Arguments:
  <inputFolder>  (Required) The path to the folder containing input JPEG files.
  <outputFolder> (Required) The path to the folder where processed JPEG files will be saved.
...
Outputs:
  ...
  - Processed JPEG files: Saved to the specified <outputFolder> with updated metadata.

Example:
  node ${scriptName} ./input-jpegs ./output-jpegs
  node ${scriptName} ./input-jpegs ./output-jpegs -v
...
if (args.length < 2 || args.length > 3) {
    console.error('Usage: node updateJpegMetadata.js <inputFolder> <outputFolder> [-v]');
    process.exit(1);
}
...
// Metadata constants
const TITLE = "White Kitchen JPEG";
...
function updateJpegMetadata(inputPath, outputPath) {
...
// Function to process all JPEG files in the input folder
function processJpegFolder(inputFolder, outputFolder) {
    ...
    if (files.length === 0) {
        logToFile(`No JPEG files found in ${inputFolder}`);
        return;
    }
    ...
    files.forEach(file => {
        ...
        updateJpegMetadata(inputFilePath, outputFilePath);
    });

    logToFile('All JPEG files processed.');
    logToFile('Script completed');
}

// Run the script
processJpegFolder(INPUT_FOLDER, OUTPUT_FOLDER);