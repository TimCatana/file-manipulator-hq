...
async function processWebmToMp4(inputFile, outputFile) {
...
async function convertWebmToMp4(args = process.argv.slice(2)) {
  try {
    ...
    if (stats.isFile()) {
      ...
      await processWebmToMp4(inputPath, outputFile);
      ...
      for (const file of webmFiles) {
        ...
        log('DEBUG', `Generated output filename: ${outputFile}`);
        await processWebmToMp4(inputFile, outputFile);
      }
      log('INFO', `Processed ${webmFiles.length} WebM files to MP4.`);
    }

    log('DEBUG', 'WebM to MP4 Conversion completed successfully');
    return 'success';
  } catch (error) {
    log('ERROR', `Unexpected error in WebM to MP4 Conversion: ${error.message}`);
